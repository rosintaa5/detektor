...
export default function HomePage() {
  ...
  const buildDepthSignal = useCallback(
    (coin: CoinSignal, depth: DepthResponse): DepthSignal | null => {
      ...
      const confidencePct = Math.min(99, Math.max(55, Math.round(score * 0.95 + (wallNear ? 4 : 0))));
      const waitLabel = confidencePct >= 75 ? 'Siap pantau' : 'Wait dulu';
                  ...
                  <tbody>
                    {depthSignals.map((item, idx) => (
                      <tr key={item.pair}>
                        ...
                        <td>
                          <div className="depth-score">{item.confidencePct}%</div>
                          <div className={`depth-wait ${item.waitLabel === 'Wait dulu' ? 'is-wait' : 'is-ready'}`}>
                            {item.waitLabel}
                          </div>
                        </td>
                        <td>
