...
export default function HomePage() {
  ...
  const handleOrderScan = useCallback(async () => {
    ...
    try {
      ...
      const [summariesRes, pairsRes] = await Promise.all([
        fetch('/api/indodax/summaries'),
        fetch('/api/indodax/pairs'),
      ]);
      if (!summariesRes.ok) {
        throw new Error(`Summaries gagal ${summariesRes.status}`);
      }
      if (!pairsRes.ok) {
        throw new Error(`Pairs gagal ${pairsRes.status}`);
      }
      const summaries = (await summariesRes.json()) as SummariesResponse;
      const pairList = (await pairsRes.json()) as Array<{ id: string; ticker_id?: string }>;
      const pairMap = new Map(
        pairList
          .filter((p) => p.ticker_id && p.id)
          .map((p) => [p.ticker_id as string, p.id as string])
      );
      const tickers = summaries.tickers ?? {};
      const topPairs = Object.entries(tickers)
        .map(([pair, info]) => ({
          pair,
          pairId: pairMap.get(pair) ?? pair.replace('_', ''),
      ...
      const results = await Promise.allSettled(
        topPairs.map(async (item) => {
          ...
          const depthRatio = sellValue > 0 ? buyValue / sellValue : buyValue > 0 ? 5 : 1;

          const trades = tradesData.slice(0, 500);
          const lastTrade = trades[0];
          const lastPrice = lastTrade ? Number(lastTrade.price) : last;
          const lastTimestamp = lastTrade ? Number(lastTrade.date) * 1000 : Date.now();
          const oldStart = lastTimestamp - 60 * 60 * 1000;
          const oldEnd = lastTimestamp - 30 * 60 * 1000;

          const oldTrades = trades.filter((t) => {
            const ts = Number(t.date) * 1000;
            return ts >= oldStart && ts <= oldEnd;
          });
          const newTrades = trades.filter((t) => {
            const ts = Number(t.date) * 1000;
            return ts > oldEnd && ts <= lastTimestamp;
          });

          const volOld = oldTrades.reduce((sum, t) => sum + Number(t.amount), 0);
          const volNew = newTrades.reduce((sum, t) => sum + Number(t.amount), 0);
          const volSpike = volOld > 0 ? volNew > volOld * 3 : volNew > 0;

          const oldPrices = oldTrades.map((t) => Number(t.price));
          const priceMinOld = oldPrices.length ? Math.min(...oldPrices) : lastPrice;
          const priceMaxOld = oldPrices.length ? Math.max(...oldPrices) : lastPrice;
          const rangePct = priceMinOld > 0 ? ((priceMaxOld - priceMinOld) / priceMinOld) * 100 : 0;
          const rangeTight = rangePct < 5;
          const breakout = rangeTight && lastPrice > priceMaxOld * 1.03;
          ...
          const volumeScore = volSpike ? 30 : 10;
          const depthScore = depthRatio > 2 ? 30 : depthRatio > 1.2 ? 15 : 5;
          const breakoutScore = breakout ? 30 : 10;
          const score = Math.round(Math.min(100, volumeScore + depthScore + breakoutScore));

          const action = volSpike && breakout && depthRatio > 2 ? '🔥 Strong candidate' : '⚠️ Watchlist';
          const reason = `Vol spike ${volSpike ? 'YES' : 'NO'} • Breakout ${breakout ? 'YES' : 'NO'} • Depth ${depthRatio.toFixed(2)}x`;
          const entry = last * 0.995;
          const sl = last * 0.97;
          const tp = last * 1.06;
          const tradeHint = action.includes('Strong')
            ? 'Tekanan beli kuat, bisa entry bertahap.'
            : 'Masih butuh konfirmasi, entry kecil atau tunggu.';
          ...
          return {
            ...
            wallNote: `Buy wall ${formatPrice(wallBuy.price)} • Sell wall ${formatPrice(wallSell.price)}`,
            entry,
            tp,
            sl,
            tradeHint,
                    ...
                    {orderSignals.slice(0, 10).map((item) => (
                      <tr key={`order-${item.pair}`}>
                        ...
                        <td>
                          ...
                          <div className="safe-sub">{item.label}</div>
                        </td>
                        <td>
                        ...
                        <td>
                          <div className="safe-score">Spike {item.volSpike ? 'YES' : 'NO'}</div>
                          <div className="safe-sub">Range {item.rangePct.toFixed(2)}% • Breakout {item.breakout ? 'YES' : 'NO'}</div>
                        </td>
                        <td>
                          <div className="safe-score">{item.depthRatio.toFixed(2)}x</div>
                          <div className="safe-sub">Buy vs sell 3%</div>
                        </td>
                        <td>
                          <div className="safe-reason">{item.wallNote}</div>
                          <div className="safe-sub">Buy {formatPrice(item.topBuyPrice)} • Sell {formatPrice(item.topSellPrice)}</div>
                        </td>
                        <td>
